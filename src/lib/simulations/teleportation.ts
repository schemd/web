...
export interface RealQubit { readonly alpha: number; readonly beta: number }
export interface TeleportationResult { readonly input: RealQubit; readonly beforeRecovery: RealQubit; readonly afterRecovery: RealQubit; readonly fidelity: number; readonly outcomeProbability: 0.25 }
function normalize(qubit: RealQubit): RealQubit {
	if (!Number.isFinite(qubit.alpha) || !Number.isFinite(qubit.beta)) throw new RangeError('amplitudes must be finite.');
...
function x(qubit: RealQubit): RealQubit { return { alpha: qubit.beta, beta: qubit.alpha }; }
function z(qubit: RealQubit): RealQubit { return { alpha: qubit.alpha, beta: -qubit.beta }; }
export function qubitFromDegrees(degrees: number): RealQubit {
	if (!Number.isFinite(degrees) || degrees < 0 || degrees > 180) throw new RangeError('degrees must be from 0 through 180.');
...
export function teleport(inputValue: RealQubit, zBit: number, xBit: number): TeleportationResult {
	...
	if ((zBit !== 0 && zBit !== 1) || (xBit !== 0 && xBit !== 1)) throw new RangeError('measurement bits must be 0 or 1.');
	...
	return { input, beforeRecovery, afterRecovery, fidelity: Math.min(1, overlap * overlap), outcomeProbability: 0.25 };
}
