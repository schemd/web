<!-- schemd-doc: id=roadmap; label=Roadmap; title=What 0.5 shipped, and what is still open; summary=Release-ready capabilities, verification gates, publication order, and the boundaries outside the compiler.; category=Contribute; order=110 -->
...
<!-- schemd-section: id=complete; eyebrow=01 / Candidate; title=The 0.5 implementation is released; example-title=Cross-domain 0.5 specimen -->

0.5 adds relative placement and bounded rip-up routing on top of 0.4, which removed the global component and connection ceilings and added caller-controlled resource
...
bounded; and scalar/bus coercion is rejected instead of guessed. See [Limitations](/docs/0.5/overview#limitations)
and [Resource budgets](/docs/0.5/limits) before treating untrusted source as a server workload.
