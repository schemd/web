<!-- schemd-doc: id=overview; label=Quickstart; title=Start with schemd 0.5; summary=Compile multi-domain engineering diagrams into deterministic accessible SVG, and inspect the circuit behind them.; category=Get started; order=10 -->
...
<!-- schemd-section: id=migrate; eyebrow=02 / Migration; title=Move to 0.5 deliberately; example-title=Canonical gate terminals; example-summary=A gate wired through the `out` alias still emits `out1`. -->
...
That last connection is written with the `out` alias and emits `data-wire-source="X1.out1"`. Open it in the [playground](/playground/0.5.0) and read the raw SVG if that matters to us.
