...
const documents: readonly SearchDocument[] = [
	...
	},
	{
		id: 'summary',
		title: 'Sizing',
		summary: 'Responsive SVG vectors',
		category: 'B',
		path: '/d',
		text: 'render output'
	},
	...
	it('normalizes, deduplicates, bounds, and filters query tokens', () => {
		expect(
			normalizeSearchQuery(
				'  SVG svg @schemd/core ! a responsive.output extra one two three four  '
			)
		).toEqual(['svg', '@schemdcore', 'responsive.output', 'extra', 'one', 'two', 'three', 'four']);
		expect(normalizeSearchQuery('! a')).toEqual([]);
	});

	it('scores every match tier and sorts deterministically', () => {
		expect(searchDocuments(documents, 'svg').map((result) => result.id)).toEqual([
			'exact',
			'prefix',
			'title',
			'summary',
		...
		expect(searchDocuments(documents, 'render output').map((result) => result.id)).toEqual([
			'prefix',
			'text',
			'title',
			'summary',
