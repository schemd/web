<script lang="ts">
	...
	const statusGroups = [
		{ status: 'planned', label: 'Planned work', note: 'Ordered by the current core roadmap.' },
		{
			status: 'active',
			label: 'Active work',
			note: 'No roadmap item is publicly claimed right now.'
		},
		{
			status: 'blocked',
			label: 'Blocked work',
			note: 'No roadmap item is recorded as blocked right now.'
		}
...
<div class="timeline-page page-shell">
	<header class="timeline-hero">
		...
		<p class="lede">
			This is an active engineering queue, not a set of release promises. Priorities describe
			correctness and leverage; expected releases stay unset until the work is understood.
		</p>
		<div class="timeline-source">
			<span class="status-dot"></span>
			<p>
				Generated from <a
					href={`${ROADMAP_SOURCE.repository}/blob/${ROADMAP_SOURCE.commit}/${ROADMAP_SOURCE.path}`}
					>core ROADMAP.md at {ROADMAP_SOURCE.commit}</a
				>
				on {ROADMAP_SOURCE.generatedAt}.
			</p>
		</div>
	</header>
	...
	<section class="phase-map" aria-labelledby="phase-title">
		...
		<ol>
			<li>
				<span>Now</span><strong>Topology + routing</strong><small>Correctness foundation</small>
			</li>
			<li><span>Next</span><strong>Visual precision</strong><small>Professional output</small></li>
			<li>
				<span>Later</span><strong>Language + footprint</strong><small>Measured expansion</small>
			</li>
		</ol>
	</section>
			...
			{#if items.length > 0}
				<div class="roadmap-table" role="region" aria-label={`${group.label} table`}>
					<table>
						<thead
							><tr
								><th scope="col">ID</th><th scope="col">Work</th><th scope="col">Phase</th><th
									scope="col">Expected</th
								><th scope="col">Claim</th></tr
							></thead
						>
						<tbody>
							{#each items as item}
								<tr>
									...
									<td
										><strong>{item.title}</strong><small>{item.scope}</small
										>{#if item.dependsOn.length}<small
												>Depends on {item.dependsOn.join(', ')}.</small
											>{/if}</td
									>
									<td>{item.phase}</td>
	...
	<section class="status-section completed" aria-labelledby="completed-title">
		<header>
			...
			<h2 id="completed-title">Completed releases</h2>
			<p>
				Finished roadmap defects leave the active queue. Tags, merged work, and Git history keep the
				record.
			</p>
		</header>
		<ol class="release-line">
			{#each RELEASES as release}
				<li>
					<time datetime={release.date}>{release.date}</time><span aria-hidden="true"></span>
					<div>
						<h3><a href={release.tagUrl}>{release.version} · {release.title}</a></h3>
						<p>{release.summary}</p>
					</div>
				</li>
			{/each}
	...
	<section class="contribute">
		...
		<p>
			That keeps parallel work from colliding. A proposal must preserve zero runtime dependencies,
			the 20 KiB compiler gzip budget, deterministic output, and full meaningful core coverage.
		</p>
		<a class="button button--signal" href="https://github.com/schemd/core/issues"
			>Browse core issues</a
		>
	</section>
</div>
...
<style>
	...
	.phase-map li:not(:last-child)::after {
		position: absolute;
		z-index: 1;
		...
		block-size: 0.65rem;
		rotate: 45deg;
		border-block-start: 1px solid var(--signal);
		border-inline-end: 1px solid var(--signal);
	...
	.status-section > header > p:last-child {
		grid-column: 2;
		grid-row: 1 / span 2;
		align-self: end;
		color: var(--muted);
	...
	.roadmap-table td:first-child span {
	...
	.roadmap-table td:first-child .priority-two {
	...
	.release-line li > span {
		position: relative;
		border-inline-start: 1px solid var(--line);
	}
	.release-line li > span::before {
		position: absolute;
