<script lang="ts">
	...
	import {
		...
		nextCompilation,
		type CompilationCoordinatorState
	} from '$lib/playground/coordinator';
	...
	import { insertIndent, workspaceLines } from '$lib/playground/workspace';
	...
	let coordinator: CompilationCoordinatorState = createCoordinatorState();
	...
	let highlightedLines = $derived(workspaceLines(source));
	let schema = $derived(
		breadcrumbSchema([
			{ name: 'Schemd', path: '/' },
			{ name: `Playground ${data.version.id}`, path: canonicalPath }
		])
	);
	...
	function focusDiagnosticLine(): void {
		...
		let start = 0;
		for (let index = 1; index < diagnostic.line; index += 1)
...
<Seo
	title={`Playground ${data.version.id} — Schemd`}
	description="Edit Schemd source, compile in an isolated worker, inspect diagnostics, and export deterministic SVG."
	path={canonicalPath}
...
<div class="playground-page page-shell">
	<header class="playground-header">
		<div class="flow">
			<nav class="breadcrumbs" aria-label="Breadcrumb">
				<ol>
					<li><a href="/">Home</a></li>
					<li aria-current="page">Playground {data.version.id}</li>
				</ol>
			</nav>
			<p class="eyebrow">Worker-isolated compiler</p>
			<h1>Write the source. Inspect the vector.</h1>
			<p class="lede">
				The textarea stays light. Compilation is debounced, bounded, and moved off the main thread.
			</p>
		</div>
		<form action="/playground/select" method="get">
			<label for="playground-version">Compiler version</label>
			<div>
				<select id="playground-version" name="version"
	...
	<section class="workbench" aria-label="Schemd workbench">
		<div class="editor-panel">
			<div class="panel-bar">
				<div><span class="panel-index">01</span><strong>Source</strong></div>
				<span>{source.length.toLocaleString()} / {data.maximumCharacters.toLocaleString()}</span>
			</div>
			<div class="editor-tools">
				<label for="example-select">Example</label>
				<select id="example-select" bind:value={selectedExample} onchange={loadExample}
					>{#each data.examples as example}<option value={example.id}>{example.label}</option
						>{/each}</select
				>
			...
			<div class="workspace-editor">
				<div class="line-numbers" aria-hidden="true" style={`translate:0 ${-editorScrollTop}px`}>
					{#each highlightedLines as line (line.number)}<span>{line.number}</span>{/each}
				</div>
				<pre
					class="syntax-overlay"
					aria-hidden="true"
					style={`translate:${-editorScrollLeft}px ${-editorScrollTop}px`}><code
						>{#each highlightedLines as line (line.number)}<span class="workspace-line"
								>{#each line.tokens as token}<span
										class={`workspace-token workspace-token--${token.kind}`}>{token.value}</span
									>{/each}</span
							>{/each}</code
					></pre>
				<textarea
					...
					autocapitalize="off"
					autocomplete="off"
					aria-describedby="editor-help editor-shortcuts editor-diagnostic"
			...
			<p id="editor-shortcuts" class="visually-hidden">
				Tab inserts two spaces. Command or Control plus Enter compiles immediately. Command or
				Control plus S downloads the current SVG.
			</p>
			<div id="editor-help" class="editor-help">
				<span
					>{sourceNearLimit
						? 'Approaching the source limit.'
						: 'Coordinates use the selected example bounds.'}</span
				><span>{compiling ? 'Compiling…' : '180 ms debounce'}</span>
			</div>
			<div id="editor-diagnostic" class:has-error={diagnostic} class="diagnostic">
				{#if diagnostic}<button type="button" onclick={focusDiagnosticLine}
						><strong>{diagnostic.line ? `Line ${diagnostic.line}` : 'Compiler error'}</strong><span
							>{diagnostic.message}</span
						></button
					>{:else if sourceEmpty}<p>Add at least one component declaration.</p>{:else}<p>
						<span class="status-dot"></span> No compiler diagnostics.
					</p>{/if}
			</div>
		</div>
		...
		<div class="preview-panel">
			<div class="panel-bar">
				<div><span class="panel-index">02</span><strong>SVG preview</strong></div>
				<span>{metrics.svgBytes.toLocaleString()} bytes</span>
			</div>
			<div class="preview-tools">
				<label for="preview-scale">Preview size</label><select
					id="preview-scale"
					bind:value={previewScale}
					><option value="fit">Fit</option><option value="100">100%</option><option value="200"
						>200%</option
					></select
				>
				<button type="button" onclick={() => copyText(previewSvg, 'SVG')} disabled={!previewSvg}
					>Copy SVG</button
				>
			...
			<div
				...
				class:scale-200={previewScale === '200'}
				class="preview-stage"
			>
				{#if workerFailed}<div class="preview-state">
						<p>The compiler worker stopped before it could return a result.</p>
						<button type="button" onclick={startWorker}>Restart worker</button>
					</div>{:else if sourceEmpty}<div class="preview-state">
						<p>The preview is empty because the editor has no component declarations.</p>
					</div>{:else if previewSvg}<iframe
						title={`SVG preview: ${activeExample.title}`}
						sandbox=""
						srcdoc={previewSvg}
					></iframe>{:else}<div class="preview-state">
						<p>{compiling ? 'Loading the versioned compiler…' : 'No SVG is available.'}</p>
					</div>{/if}
			</div>
			<dl class="preview-metrics">
				...
				<div>
					<dt>Connections</dt>
					<dd>{metrics.connections}</dd>
				</div>
				<div>
					<dt>Source</dt>
					<dd>{metrics.sourceCharacters} chars</dd>
				</div>
				<div>
					<dt>Output</dt>
					<dd>{metrics.svgBytes} B</dd>
				</div>
			</dl>
		</div>
	</section>
	<p class="playground-note">
		Preview content is compiler output rendered inside a sandboxed frame. <a
			href={`/docs/${data.version.id}/grammar`}>Read the grammar and diagnostics contract.</a
		>
	</p>
...
<style>
	...
	.playground-header form label {
		color: var(--muted);
		font-size: 0.72rem;
		text-transform: uppercase;
	}
	.playground-header form > div {
		display: flex;
		gap: 0.4rem;
	}
	...
	.workspace-editor textarea::selection {
		background: rgb(114 219 187 / 0.28);
	}
	.syntax-overlay {
		z-index: 1;
		overflow: visible;
	...
	.workspace-line {
		display: block;
		min-block-size: calc(0.78rem * 1.75);
	}
	.line-numbers {
		display: grid;
		position: absolute;
		...
		font-family: var(--font-mono);
		font-size: 0.68rem;
		line-height: calc(0.78rem * 1.75);
		pointer-events: none;
	}
	.line-numbers span {
		block-size: calc(0.78rem * 1.75);
	}
	.workspace-token--kind {
		color: var(--purple);
	}
	.workspace-token--identifier {
		color: var(--paper-soft);
	}
	.workspace-token--string {
		color: var(--signal-bright);
	}
	.workspace-token--number {
		color: var(--amber);
	}
	.workspace-token--operator,
	.workspace-token--attribute {
		color: var(--blue);
	}
	.workspace-token--comment {
		color: var(--muted);
	}
	.editor-help {
		justify-content: space-between;
	...
	.diagnostic.has-error {
		border-color: var(--danger);
		background: rgb(255 143 123 / 0.06);
	}
	.diagnostic button {
		display: grid;
		justify-items: start;
	...
	.preview-stage.scale-100 iframe {
		inline-size: 50rem;
		max-inline-size: none;
	}
	.preview-stage.scale-200 iframe {
		inline-size: 100rem;
		max-inline-size: none;
	}
	.preview-state {
	...
	@media (max-width: 40rem) {
		...
		.preview-metrics div:nth-child(4) {
			border-block-start: 1px solid var(--line);
		}
	}
