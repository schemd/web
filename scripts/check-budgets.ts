...
if (cssGzip > MAX_INITIAL_CSS_GZIP) throw new Error(`Combined first-party CSS is ${cssGzip} B gzip, above ${MAX_INITIAL_CSS_GZIP} B.`);

const workerFiles = files.filter((path) => path.includes('/workers/') && path.endsWith('.js'));
if (workerFiles.length !== 1) throw new Error(`Expected one versioned compiler worker; found ${workerFiles.length}.`);
const worker = await readFile(workerFiles[0]);
const workerGzip = gzipSync(worker, { level: 9 }).byteLength;
if (workerGzip > MAX_COMPILER_GZIP) throw new Error(`Compiler worker is ${workerGzip} B gzip, above ${MAX_COMPILER_GZIP} B.`);

const browserJavaScript = files.filter((path) => path.endsWith('.js') && !path.includes('/workers/'));
...
if (/<script\s+[^>]*src=["']https?:\/\//iu.test(appTemplate)) throw new Error('A third-party runtime script was added to app.html.');
